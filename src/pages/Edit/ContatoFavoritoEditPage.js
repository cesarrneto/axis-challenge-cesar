...
const ContatoFavoritoEditPage = () => {
  ...
  const handleSubmit = async (values) => {
    setLoading(true);
    try {
      await api.put(`/contatofavorito/${id}`, {
        ...values,
      ...
      message.error("Erro ao atualizar contato favorito.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card title="Editar Contato Favorito" style={{ maxWidth: 500, margin: "auto" }}>
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item>
          <Button type="primary" htmlType="submit" block style={{ backgroundColor: "#fe3f00", borderColor: "#fe3f00" }} loading={loading}>
            Salvar Alterações
          </Button>
        </Form.Item>
      </Form>
